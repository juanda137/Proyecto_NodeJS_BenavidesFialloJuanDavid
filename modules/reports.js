...
/** REPORTE 1: CORREGIDO Y VERIFICADO */
export async function getActiveEmployeesByAreaAndPosition() {
    const pipeline = [
        ...
        // ✅ CORRECCIÓN DEFINITIVA: Se usa "cargo" en singular.
...
/** REPORTE 2: CORREGIDO Y VERIFICADO */
export async function getPayrollDetailsForEmployee(employeeId, payrollId) {
    const pipeline = [
        ...
        { $unwind: '$empleadoInfo' },
        { $project: { _id: 0, tipo_identificacion: '$empleadoInfo.tipo_identificacion', numero_identificacion: '$empleadoInfo.numero_identificacion', nombres: '$empleadoInfo.nombres', apellidos: '$empleadoInfo.apellidos', salario_base: '$empleadoInfo.contrato.salario_base', devengos: 1, deducciones: 1 } }
    ];
    return runAggregation('desprendibles', pipeline);
}

/** REPORTE 3: CORREGIDO Y VERIFICADO */
...
/** REPORTE 4: Sin cambios, no usa la colección de cargos. */
